...
** Snapshot IDs
- Each check assigns a =snapshot_id= to the AST snapshot used to derive actions.
- The =snapshot_id= is returned alongside actions and sent back on =apply_action=.

** Server-side registries
When responding to =check=:
- The server registers the snapshot in a snapshot registry keyed by =snapshot_id=.
- The server registers each proposed action in an action registry keyed by =Action_id=.
- Action registry entry stores:
  - the =apply= function (closure) from =Actions_core=
  - =snapshot_id= for lookup
...
- =action_id=
- =snapshot_id=
- =text= (current buffer contents; required for =edits=, otherwise server returns =replace_all=)
- =version=
- =content_hash=
...
3) load snapshot via snapshot_id
4) apply rewrite to AST
5) pretty-print/format new text
6) compute patch from old text to new text (old text comes from request =text=)
7) respond with patch (=edits= when possible, else =replace_all=)
